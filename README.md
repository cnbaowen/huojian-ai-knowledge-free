...
默认的本地抽取模式不调用外部模型，安装后即可完成上传、检索、引用问答和评测，适合企业内网和离线环境。
...
- 希望低成本建设企业知识问答能力的团队
- 希望在自己的服务器或内网环境中部署知识库的企业

免费版聚焦企业资料管理、内部知识查询和有据可查的问答，可部署在企业自己的服务器或内网环境中。系统采用部署时设置的访问令牌登录，安装简单，资料和数据由企业自行管理。
